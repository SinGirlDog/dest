...
if(!empty($tid))
{
    ...
    $lv_cross = new ListView_cross($tid);
    $test_id = $lv_cross->GetCrossId($tid);
    
    //如果栏目的文档太多，分多批次更新
    if($ntotalpage <= $maxpagesize || $lv->TypeLink->TypeInfos['ispart']!=0 || $lv->TypeLink->TypeInfos['isdefault']==-1)
    {
        if($tid != $test_id){
            $reurl_cross = $lv_cross->MakeHtml('', '', $isremote);
        }
        else{
    ...
    {
        if($tid != $test_id){
            $reurl_cross = $lv_cross->MakeHtml($mkpage, $maxpagesize, $isremote);
        }
        else{
