...
require_once(DEDEINC."/arc.listview_cross.class.php");
require_once(DEDEINC."/arc.archives_cross.class.php");
...
$totalnum_cross = (empty($totalnum_cross) ? 0  : $totalnum_cross);
...
//创建listview类预算交叉栏目id
$lv_cross = new listview_cross($typeid);
if($lv_cross->TypeID != $typeid){
    $crossid = $lv_cross->TypeID;
}
else{
    $crossid = 0;
}
...
//也获取交叉栏目的相关条件
if($crossid != 0){
    $ids_cross = GetSonIds($crossid);
    $gwhere_cross .= $gwhere." AND typeid in($ids_cross) ";
}
if($typeid!=0) {
...
if($seltime==1)
{
    ...
    if($crossid != 0){
        $idsql_cross .= " And (senddate >= $t1 And senddate <= $t2) ";
    }
}
...
//统计记录总数
if($totalnum_cross==0 and $crossid != 0){
   $row_cross = $dsql->GetOne("SELECT COUNT(*) AS dd_cross FROM `#@__arctiny` $idsql_cross");
   $totalnum_cross = $row_cross['dd_cross'];
    //清空缓存
   $dsql->ExecuteNoneQuery("DELETE FROM `#@__arccache` ");
}
...
//也获取交叉栏目记录
if($totalnum_cross > $startdd+$pagesize and $crossid != 0) 
{
    $limitSql_corss = " LIMIT $startdd,$pagesize";
}
else {
    $limitSql_corss = " LIMIT $startdd,".($totalnum_cross - $startdd);
}
...
//如果交叉栏目生成数量大于500，并且没选栏目，就什么都不处理
if($totalnum_cross > 500 && empty($typeid)) 
{
    $dsql->Execute('out_cross',"SELECT id FROM `#@__arctiny` $idsql where id < '0' ");
} else {
    //进行到此应该是已经选择栏目，因此才能考虑交叉
    $dsql->Execute('out_cross',"SELECT id FROM `#@__arctiny` $idsql_cross $limitSql_corss");
}
...
if($crossid != 0){
    while($row=$dsql->GetObject('out_cross'))
    {
        $tjnum_cross++;
        $id = $row->id;
        $ac_cross = new Archives_cross($id,$typeid);
        $rurl_cross = $ac_cross->MakeHtml($isremote);
    }
}
...
//返回提示信息
$tjlen = $totalnum>0 ? ceil( (($tjnum+$tjnum_cross)/$totalnum) * 100 ) : 100;
$dvlen = $tjlen * 2;
