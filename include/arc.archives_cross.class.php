 ...
 * 文档类
 *
 * @version        $Id: arc.archives_mobile.class.php 4 09:28 2018年9月9日Z edit by SLJ $
...
class Archives_cross
{
     ...
     * @access    public
     * @param     int  $aid  文档ID
     * @param     int  $orig_typeid  交叉原ID
     * @return    string
     */
    function __construct($aid,$orig_typeid)
    {
        global $dsql,$ftp;
        $this->IsError = FALSE;
        $this->ArcID = $aid;
        ...
        {
            $this->typeID_orig = $orig_typeid;
            $arr['typeid'] = $this->GetCrossId($orig_typeid);
            $this->typeID_new = $arr['typeid'];

            if($arr['channel']==0) $arr['channel']=1;
            $this->ChannelUnit = new ChannelUnit($arr['channel'], $aid);
            $this->TypeLink = new TypeLink($orig_typeid);
    ...
    {
        //交叉文档偷懒处理next和pre处理
        $the_typedir = $this->GetCrossOrigTypedir();
    ...
    /**
     * 获取交叉栏目的id
     * @param int $typeid original
     * @return int $crossid
     *
     */
    function GetCrossId($typeid)
    {
         $query = "SELECT id,crossid FROM `#@__arctype` WHERE id = '".$typeid."' ";
         $this->dsql->SetQuery($query);
         $this->dsql->Execute();
         $arr=$this->dsql->GetOne();
         if($arr['crossid']) return $arr['crossid'];
         else return $arr['id'];
    }

    /**
     * 验证是否需要更换栏目ID来生成来自交叉栏目的文档
     * @param int $typeid original new
     * @return bool $typeid afterchange
     *
     */
    function IsCross(){
        if($this->typeID_orig == $this->typeID_new){
            return false;
        }
        else{
            return true;
    ...
    /**
     *  获得指定ID的typeidr
     *  若是 交叉 栏目需要取 原栏目文档，则此函数获取交叉路径
     *  若无需交叉，则此函数取得就是原路径，不影响生成
     * @access    public
     * @return    string typeidr
     */
    function GetCrossOrigTypedir(){
        $orig_typeid = $this->typeID_orig;

        $query = "SELECT id,typedir FROM `#@__arctype` WHERE id = '".$orig_typeid."' ";
        $this->dsql->SetQuery($query);
        $this->dsql->Execute();
        $arr=$this->dsql->GetOne();
        return $arr['typedir'];
