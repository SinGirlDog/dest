        ...
        $innertext='', $arcid=0, $idlist='', $channelid=0, $limit='', $att='', $order='desc', $subday=0, $noflag='',$tagid='', $pagesize=0, $isweight='N')
{
        ...
        if(!empty($typeid) && $typeid != 'top')
        {
            ...
            if( preg_match('#,#', $typeid) )
            {
                ...
                if($getall==1 || empty($refObj->Fields['typeid']))
                {
                    ...
                    //存储一下原始tiyeid数组,似乎没什么用啊
                    $orig_typeid_arr = $typeidssok;
                }
                ...
                if($ctag->GetAtt('cross')!='0')
                {
                    ...
                    else
                    {
                        ...
                        if($arr['cross']==1) {
                            ...
                            //存储一下原始tiyeid数组,计划以原typedir替换交叉栏目路径
                            $typeid_crossid_arr[$typeid] = explode(',', $arr['crossid']);
                        }
                        if($selquery!='')
                        {
    ...
    $query = "SELECT arc.*,tp.typedir,tp.typename,tp.corank,tp.isdefault,tp.defaultname,tp.namerule,
        tp.namerule2,tp.ispart,tp.moresite,tp.siteurl,tp.sitepath,tp.cross,tp.crossid
        $addfieldsSql
    ...
    if($idlist != '')
    {
        $query = "SELECT arc.*,tp.typedir,tp.typename,tp.corank,tp.isdefault,tp.defaultname,tp.namerule,tp.namerule2,tp.ispart,
            tp.moresite,tp.siteurl,tp.sitepath,tp.cross,tp.crossid
            $addfieldsSql
             FROM `$maintable` arc left join `#@__arctype` tp on arc.typeid=tp.id
             $addfieldsSqlJoin
    ...
    for($i=0; $i<$line; $i++)
    {
        ...
        for($j=0; $j<$col; $j++)
        {
            ...
            if($row = $dsql->GetArray("al"))
            {
                ...
                //替换交缠栏目url为原路径-SLJ
                if(is_array($typeid_crossid_arr)){
                    foreach($typeid_crossid_arr as $typeid_orig => $crossid_arr)
                    if(in_array($row['typeid'], $crossid_arr)){
                        $row['typedir'] = get_cross_orig_typedir($typeid_orig);
                    }
                }
...
function list_sort_by($list, $field, $sortby='asc') {
 ...
 /**
     ...
     */
    function get_cross_orig_typedir($orig_typeid){
        global $dsql;
